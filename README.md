...
```
同一容器
├── Qdrant      :6333  （向量库，挂载到宿主机 ../qdrant_data → 容器 /data/qdrant）
└── 控制台 BFF  :8787  （前端 + API，用户卷 /data/config）
         ↓
    外部 Embedding（宿主机或另一容器，EMBED_URL 配置）
```

目录示例（`qdrant_data` 与项目仓库同级）：

```
/path/to/
├── qdrantUI/          # 本项目（git clone 目录）
└── qdrant_data/       # Qdrant 向量数据（默认挂载到这里）
```
...
| `QDRANT_DATA_DIR` | 宿主机 Qdrant 数据目录（相对 compose 文件） | `../qdrant_data` |
...
| `Qdrant 进程异常退出` | 数据目录损坏或权限问题 | 停止容器后清空宿主机 `../qdrant_data` 重试（会删向量数据）；或 `docker compose down -v` 清空账号卷 |
...
| **数据持久化** | Qdrant 数据默认绑定到宿主机 `../qdrant_data`；账号在卷 `app_config`。`docker compose down` 不删数据目录 |
